...
impl<'a> Player<'a> {
	...
	fn y(&self) -> i32 {self.pos.y()}
	fn width(&self) -> u32 {self.pos.width()}
	fn height(&self) -> u32 {self.pos.height()}
...
impl Demo for SDL04 {
	...
	fn run(&mut self) -> Result<(), String> {
		...
		let background_image = texture_creator.load_texture("images/MapPlaceholder.png")?;
		...
		'gameloop: loop {
			...
			p.update_pos(speed_update, 
				x_limits,
				(0, ((BG_H + (SCALE_UP * TILE_SIZE as i16) as u32) * (SCALE_UP as u32) / 2) as i32),
			);
			
			
			// Determine the current portion of the background to draw
			let cur_bg = Rect::new(
				((p.x() + ((p.width() / 2) as i32)) - ((CAM_W / 2) as i32)).clamp(0, (BG_W - (CAM_W / SCALE_UP as u32)) as i32),
				((p.y() + ((p.height() / 2) as i32)) - ((CAM_H / 2) as i32)).clamp(0, (BG_H - (CAM_H / SCALE_UP as u32)) as i32),
